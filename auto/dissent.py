...
class GRUDecoder(nn.Module):
    def __init__(self, config):
        # this version of decoder, compression is mandatory
        super(GRUDecoder, self).__init__()
        self.bsize = config['bsize']
        self.word_emb_dim = config['word_emb_dim']
        self.enc_lstm_dim = config['enc_lstm_dim']
        self.dec_lstm_dim = config['dec_lstm_dim']
        self.dpout_model = config['dpout_decoder']
        self.n_words = config['n_words']
        self.tied_emb = config['tied_emb']

        # self.dec_lstm = nn.GRU(self.word_emb_dim, self.dec_lstm_dim, 1,
        #                         bidirectional=False, dropout=self.dpout_model)

        # if we want to do tied-embedding...
        self.w_softmax = None if self.tied_emb else nn.Parameter()

        self.dec_lstm = nn.GRUCell(self.word_emb_dim, self.dec_lstm_dim, True)

    def is_cuda(self):
        # either all weights are on cpu or they are on gpu
        return 'cuda' in str(type(self.enc_lstm.bias_hh_l0.data))

    def init_dec_vocab(self, vocab_emb):
        self.w_softmax = vocab_emb  # <p> is our padding token

    def forward(self, h, sent_tuple):
        # sent_tuple: (s1, s1_len)
        # sent_len: [max_len, ..., min_len] (bsize) (numpy array)
        # sent: Variable(seqlen x bsize x worddim)
        sent, sent_len = sent_tuple

        # sent here is not needed, only sent_len
...
class DisSent(nn.Module):
    def __init__(self, config):
        ...
        self.decoder = eval(self.decoder_type)(config)
    ...
    def forward(self, s1, s2, s1_tgt, s2_tgt):
        # s1 : (s1, s1_len)
        u = self.encoder(s1)
        v = self.encoder(s2)
        # u: (batch_size, 2 * lstm_dim), typically (32, 4096 * 2)
